...
from .qa_engine import answer_question  # <<— now comes from Utilities core
...
def gen_answer(
    ...
    if indices and not choices:
        history_block = _build_history(indices)
        followup_prompt = (
            "You previously answered these questions:\n"
            f"{history_block}\n\n"
            "Use that context to answer the follow-up question below.\n"
            "If the earlier answer already contains the information, summarize it concisely. "
            "Do not fabricate new sources—stick to the provided context.\n\n"
            f"Follow-up question: {question}"
        )
        result = _llm_client.get_completion(followup_prompt)
        ans = result[0] if isinstance(result, tuple) else result
        cmts = []
