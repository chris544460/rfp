#!/usr/bin/env python3
"""Quick CLI to exercise the FAISS retrieval stack."""
...
from scripts.retrieval_smoke_utils import load_faiss_stack


def _parse_args(argv: Optional[Iterable[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Query the local FAISS vector store and print the top matches.",
    )
    parser.add_argument("query", help="Query text to embed and search for.")
    parser.add_argument(
        "--mode",
        default="dual",
        choices=("answer", "question", "blend", "dual"),
        help="Retrieval mode to use (default: dual).",
    )
    parser.add_argument(
        "--k",
        type=int,
    ...
    parser.add_argument(
        "--fund",
        dest="fund_filter",
        help="Optional fund tag to filter results.",
    )
    parser.add_argument(
        "--include-vectors",
        action="store_true",
...
def _print_hits(hits: Iterable[Dict[str, Any]], *, raw: bool, include_vectors: bool) -> None:
    ...
    for idx, hit in enumerate(hits_list, start=1):
        score = hit.get("cosine", hit.get("score", "n/a"))
        try:
        ...
        origin = hit.get("origin", "faiss")
        snippet = shorten((hit.get("text") or "").strip(), width=160, placeholder="...")
        print(f"  {idx}. score={score_str} id={doc_id} origin={origin} source={source}")
